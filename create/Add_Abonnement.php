...
<div style=" background-image:radial-gradient(pink 10%,steelblue);margin-top: -20px;height: 40px; ">                
	<h3 style="margin-left: 600px;">Formulaire d'Enregistrement des Abonnés</h3>
</div>
...
<input  class="form-control" type="date" name="debut"><br>
...
if (isset($_POST['submit'])){
  ...
  if(!empty( $_POST['nom']) AND !empty( $_POST['prenom']) AND !empty( $_POST['categorie'])  AND !empty( $_POST['produit']) AND !empty($_POST['debut'])){
  	$connect = $pdo ->prepare("SELECT * FROM Clients WHERE Nom = ? AND Prenom = ?");
			$connect->execute(array($nom,$prenom));
			$rowers=$connect->rowcount();
			if($rowers>0){
  $inser=$pdo->prepare("INSERT INTO Abonnements(Nom_Client,Prenom,Categorie_Abonne,Nom_Produit,Date_Debut,Date_Fin,Date_enreg)VALUES(?,?,?,?,?,?,?)");
   $inser->execute(array($nom,$prenom,$categorie,$produit,$debut,$fin,$time));
   
   	 $succes= "Inscription réussie";
  	header('Location:../Home/Abonnement.php');

  	 }else{
 	$error= "Vous n'etes pas inscrits en tant que Client!";
		 }
