...
?>
		<div style=" background-image:radial-gradient(pink 10%,steelblue);margin-top: -20px;height: 40px; ">                
			<center><h3>Liste des Categories des Abonnées</h3></center>
		</div>
	
		<div style="background-image: radial-gradient(pink, ivory);">
		
			<a href="../Create/Add_CategorieAbonne.php"><i class="fa-solid fa-circle-plus" style="margin-left: 1600px;font-size: 30px;"></i></a>
...
<?php
 		$repons=$pdo->prepare("SELECT * FROM Categorie_Abonnes");
 		...
 		if ($row>0) {
 			?>
 			 ...
 			 <thead class="table-light">
 			 	<tr>
 			 		<th>ID</th><th>Categorie</th><th>Produit</th><th>MODIFIER</th><th>SUPPRIMER</th>
 			 	</tr>
 			 </thead>
 			 <?php
 			while($repo=$repons->fetch(PDO::FETCH_ASSOC)){
 				?>
 				<tr>
 					<td> <?php echo $repo['Id_Categorie'];?></td>
 					<td> <?php  echo  $repo['Designation'] ;?></td>
 					<td> <?php  echo  $repo['Nom_Produit'] ;?></td>

 					
 					<td> <a href ="../Modification/Change_CategorieAbonne.php?id=<?php echo $repo['Id_Categorie'];?>"> modifier<a></td>
 					<td> <a href ="../Delete/Delete_CategorieAbonne.php ?id=<?php echo $repo['Id_Categorie'];?>" onclick="return confirm('Voulez-vous supprimer?')"> Supprimer<a></td>
