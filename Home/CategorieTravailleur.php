...
?>
		<div style=" background-image:radial-gradient(pink 10%,steelblue);margin-top: -20px;height: 40px; ">
			<center><h3>Liste des Categories de Clients</h3></center>
		</div>
	
		<div style="background-image: radial-gradient(pink, ivory);">
		
			<a href="../Create/Add_CategorieTravailleur.php"><i class="fa-solid fa-circle-plus" style="margin-left: 1600px;font-size: 30px;"></i></a>
...
<?php
 		$repons=$pdo->prepare("SELECT * FROM Categorie_Travailleurs");
 		...
 		if ($row>0) {
 			...
 			while($repo=$repons->fetch(PDO::FETCH_ASSOC)){
 				...
 				<tr>
 					...
 					<td> <a href ="../Modification/Change_CategorieTravailleur.php?id=<?php echo $repo['Id_Categorie'];?>"> modifier<a></td>
 					<td> <a href ="../delete.php ?id=<?php echo $repo['Id_Categorie'];?>" onclick="return confirm('Voulez-vous supprimer?')"> Supprimer<a></td>
